...
const EvaluationDashboard: React.FC<EvaluationDashboardProps> = ({ points, onNavigate }) => {
  ...
  const validPoints = useMemo(() => {
    return points.filter(p => p.rqiScore !== undefined && p.manualRqi !== undefined && p.manualRqi !== null)
  }, [points])
  ...
  if (!metrics) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-gray-500">
        ...
        <p className="text-sm">Please manually rate some points that also have model predictions.</p>
      </div>
    )
  }
    ...
    <div className="flex flex-col h-full bg-[#111] text-white p-6 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-[#222] p-4 rounded-lg border border-[#333]">
          ...
          <div className="text-2xl font-bold">{metrics.count} <span className="text-sm font-normal text-gray-500">of {points.length}</span></div>
        </div>
        <div className="bg-[#222] p-4 rounded-lg border border-[#333]">
          <div className="text-gray-400 text-sm mb-1">MAE (Mean Absolute Error)</div>
          {/* Lower is better */}
          <div className={`text-2xl font-bold ${metrics.mae < 0.5 ? 'text-green-500' : metrics.mae < 1.0 ? 'text-yellow-500' : 'text-red-500'}`}>
        ...
        <div className="flex-1 bg-[#222] p-4 rounded-lg border border-[#333] flex flex-col items-center">
          ...
          <div className="relative">
            <svg width={CHART_SIZE} height={CHART_SIZE} className="overflow-visible">
              {/* Axes */}
              <line x1={PADDING} y1={CHART_SIZE - PADDING} x2={CHART_SIZE - PADDING} y2={CHART_SIZE - PADDING} stroke="#444" strokeWidth="2" />
              <line x1={PADDING} y1={CHART_SIZE - PADDING} x2={PADDING} y2={PADDING} stroke="#444" strokeWidth="2" />
              ...
              {[1, 2, 3, 4, 5].map(tick => {
                ...
                return (
                  <React.Fragment key={tick}>
                    {/* X Axis Ticks */}
                    <line x1={pos} y1={CHART_SIZE - PADDING} x2={pos} y2={CHART_SIZE - PADDING + 5} stroke="#666" />
                    <text x={pos} y={CHART_SIZE - PADDING + 20} textAnchor="middle" fontSize="10" fill="#888">{tick}</text>

                    {/* Y Axis Ticks */}
                    <line x1={PADDING - 5} y1={yPos} x2={PADDING} y2={yPos} stroke="#666" />
                    <text x={PADDING - 10} y={yPos + 3} textAnchor="end" fontSize="10" fill="#888">{tick}</text>
              ...
              {/* Ideal Line (Diagonal) */}
              <line 
                x1={getCoord(1)} y1={getYCoord(1)} 
                x2={getCoord(5)} y2={getYCoord(5)} 
              ...
              {validPoints.map(p => (
                ...
                >
                  <title>ID: {p.id} | Manual: {p.manualRqi} | Model: {p.rqiScore?.toFixed(2)}</title>
                </circle>
              ))}

              {/* Labels */}
              <text x={CHART_SIZE / 2} y={CHART_SIZE - 5} textAnchor="middle" fontSize="12" fill="#888">Manual RQI (Truth)</text>
              <text x={10} y={CHART_SIZE / 2} textAnchor="middle" fontSize="12" fill="#888" transform={`rotate(-90, 10, ${CHART_SIZE / 2})`}>Model RQI (Predicted)</text>
            </svg>
          </div>
          <div className="mt-4 text-xs text-gray-500 text-center">
            Points on the diagonal line are perfect predictions.<br/>
            Points above = Model overestimates damage.<br/>
                        ...
                        >
                            <td className="px-3 py-2">
                                <div className="w-10 h-8 rounded bg-gray-800 overflow-hidden border border-gray-700">
                                     {p.imageUrl ? (
                                        <img src={getImageUrl(p.imageUrl)} className="w-full h-full object-cover" />
                                     ) : <ImageIcon size={12} className="text-gray-600 m-auto mt-2" />}
                                </div>
                            </td>
                            <td className="px-3 py-2 font-mono text-xs text-gray-400">#{p.id}</td>
                            <td className="px-3 py-2 text-right font-bold text-green-400">{p.manualRqi}</td>
                            <td className="px-3 py-2 text-right text-blue-400">{p.rqiScore?.toFixed(2)}</td>
                            <td className={`px-3 py-2 text-right font-bold ${Math.abs(diff) > 1.0 ? 'text-red-500' : 'text-yellow-500'}`}>
